/**
 * The `.eventstorm` tokenizer.
 *
 * Copied from doc-sm, which is the point: the scanner knows about strings,
 * braces, sigils and positions, and nothing about either domain. Its keyword set
 * is a parameter, and swapping it is the whole difference between reading a
 * story map and reading an event storm.
 *
...
export const EVENTSTORM_KEYWORDS: ReadonlySet<string> = new Set([
	'eventstorm',
...
export function tokenize(
	source: string,
	problems: Problem[],
	keywords: ReadonlySet<string> = EVENTSTORM_KEYWORDS,
): readonly Token[] {
	const tokens: Token[] = [];
	...
	if (text.includes('\u0000')) {
		report(problems, {
			...
			column: 1,
			length: 0,
			hint: 'An event storm is plain text. Pick an `.eventstorm` file exported from this board.',
		});
	...
	if (text.length > MAX_SOURCE_BYTES) {
		report(problems, {
			...
			column: 1,
			length: 0,
			hint: 'An event storm is a handful of cards. A session that produced a megabyte of them was not an event storming session.',
		});
		return [eof(text.length, 1, 1)];
	}
