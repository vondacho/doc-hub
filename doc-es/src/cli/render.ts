/**
 * The wall, rendered without a browser.
 *
 *   render [<input>|-] [-o <output>|-] [--format svg|png|md] [--scale N]
 *
 * Reads a `.eventstorm` file (stdin when there is none, or `-`) and writes one
 * of the export dialog's wall destinations: the picture as SVG or PNG, or the
 * outline as Markdown. The format is read off the output's extension, or given
 ...
 * would be a second picture of the wall to keep in step, and the one in CI
...
import { LEVELS } from '../lib/eventstorm/model.ts';
import { parse } from '../lib/eventstorm/parser.ts';
import { EventStormParseError } from '../lib/eventstorm/problems.ts';

/** The formats this tool writes, and the export destination behind each. */
const FORMATS: Record<string, DestinationId> = { svg: 'svg', png: 'png', md: 'outline' };

const USAGE = `usage: render [<input.eventstorm>|-] [-o <output>|-] [--format ${Object.keys(FORMATS).join('|')}] [--scale N]`;
...
async function main(argv: readonly string[]): Promise<void> {
	...
	const board = toBoard(parse(source));
	const request = { board, source, level: LEVELS[0]!, matching: null, onlyShowing: false, dark: false };
...
main(process.argv.slice(2)).catch((error: unknown) => {
	if (error instanceof EventStormParseError) {
