 ...
 * row. Somebody who has used the story mapper or the example mapper should not
 * have to learn a second
 * set of gestures to use this one.
 *
 * Simpler than doc-sm's in one way — there is no ticket line — but not in
 * another: a phase header carries no fill, which arrives through
...
import { cardLabel, joinNotes, type CardKind } from '../../lib/eventstorm/model.ts';

/**
 * A phase header is drawn by this component but is not a sticky note.
 *
 * It is a boundary somebody drew on the wall, so it gets the card's editing,
 * notes and menu without the card's colour or its shadow — see `bare` below.
 */
export type CardOrPhase = CardKind | 'phase';
...
export interface CardProps {
	...
	/** Drawn without the sticky-note fill and shadow. For the phase header. */
...
export function Card({
	...
	const named = kind === 'phase' ? phaseLabel : cardLabel[kind];
	const label = `${named}: ${title}${position ? `, ${position}` : ''}`;
	...
	return (
		<div
			...
			className={`group relative text-[1em] motion-reduce:transition-none ${
				bare || kind === 'phase'
					? ''
					: `rounded-[0.4em] border px-[0.55em] py-[0.4em] shadow-sm ${cardClass[kind]}`
			} ${className}`}
		>
			{editing ? (
				<TitleEditor
					value={title}
					label={`Rename this ${named.toLowerCase()}`}
					onCommit={(next) => {
						setEditing(false);
						onRetitle(next);
					}}
