 ...
 *
 * Reads a `.storymap` file (stdin when there is none, or `-`) and writes one
 * of the export dialog's map destinations: the picture as SVG or PNG, or the
...
import { StoryMapParseError } from '../lib/storymap/problems.ts';
...
const USAGE = `usage: render [<input.storymap>|-] [-o <output>|-] [--format ${Object.keys(FORMATS).join('|')}] [--scale N]`;
...
async function main(argv: readonly string[]): Promise<void> {
	...
	const request = { board, source, matching: null, onlyShowing: false, dark: false };
...
main(process.argv.slice(2)).catch((error: unknown) => {
	if (error instanceof StoryMapParseError) {
