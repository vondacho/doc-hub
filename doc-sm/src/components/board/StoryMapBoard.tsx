...
export default function StoryMapBoard({
	...
	const createTicket = useCallback(
		async (kind: CardKind, id: Id) => {
			...
			try {
				...
				setNote({ kind: 'error', text: 'Could not reach doc-sm to raise the ticket.' });
				return;
			}

			if (!payload.id || !payload.status) {
				setNote({ kind: 'error', text: payload.error ?? 'The ticket could not be created.' });
				return;
			}
	...
	const startFresh = useCallback(() => {
		...
		const had =
			(board.activityOrder.length > 0 || board.deliveryOrder.length > 0) && board.title !== '';
	...
	}, [board.activityOrder.length, board.deliveryOrder.length, board.title, flush, load]);
