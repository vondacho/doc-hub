 ...
 *
 * Static, unlike doc-es's: a story map has three kinds and always will. It is a component only so that it can live
...
import { KIND_ORDER, kindDescription, kindLabel, swatchClass } from '../../lib/board/kinds.ts';
			...
			<ul className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
				{KIND_ORDER.map((kind) => (
					<li key={kind} className="flex items-center gap-2">
						...
						<span className="text-ink-muted dark:text-slate-400">{kindDescription[kind]}</span>
					</li>
				))}
			</ul>
		</section>
