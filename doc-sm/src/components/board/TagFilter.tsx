 ...
 * ordinary state of most maps, and a permanent strip of furniture advertising a
 * feature that has nothing to act on is exactly the noise this board spends its
 * whole design avoiding.
 *
 * ## Why dimming, and not hiding
 *
 * Argued where it is enforced — see `Card`'s note on `dimmed` — but the short
 * of it: the backbone is a narrative read left to right, and a filter that
 * removed cards from it would be answering "where is the legal work" on a map
 * whose story had holes cut in it. The board leaves the map intact and turns
 * everything else down. `filtered` in state.ts carries the other half: a hit
 * lights the step and activity above it, so an answer is never found under a
 * greyed-out heading.
 *
