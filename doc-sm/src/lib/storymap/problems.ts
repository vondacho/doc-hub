 ...
 *
 * Deliberately free of story-map vocabulary: this file knows about positions and
 * messages, not about activities or releases. It is one of the four modules
 * doc-em and doc-es will want verbatim (with lexer.ts, history.ts and files.ts),
 * so nothing domain-specific may land here.
 *
...
export function formatProblems(problems: readonly Problem[]): string {
	...
	const head = problems.length === 1 ? 'The story map file has a problem:' : `The story map file has ${problems.length} problems:`;
