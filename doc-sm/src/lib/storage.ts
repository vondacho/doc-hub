 ...
 * recoverable by hand: it is a `.storymap` file that happens to live in
 * `localStorage`, so somebody can copy it out of devtools and it is simply the
 * file.
 *
 * It also means an entry written by an older version of doc-sm is read by the
...
const LAST_OPENED = 'doc-sm:last-opened';
 ...
 * underscore.
 */
const BOARD_THEME = 'doc-sm:board-theme';
const LEGEND = 'doc-sm:legend';
const PANES = 'doc-sm:panes';
const SPLIT = 'doc-sm:split';
const EDITOR_TEXT = 'doc-sm:editor-text';
