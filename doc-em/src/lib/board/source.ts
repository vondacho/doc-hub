 ...
 *
 * The `.examplemap` file is the artefact. It lands in a pull request, it is read
...
import type { Span } from '../examplemap/model.ts';
 ...
 * line — and a block that opens on a blank line is what a freshly made rule
 ...
 * sample is written in, and the reason an `.examplemap` looks the same in a
 ...
 * will not.
 *
 * `@MVP` and `#CLONB-42` are what people write by hand, so they are what an
 * edit emits. But nothing stops a delivery being called `Q3 2026` or a tracker
 * issuing an id with a slash in it, and writing either bare would produce a file
 * this parser cannot read — so those are quoted instead of being emitted as
 * something that will not round-trip.
 *
 * This came out of the serializer when it was deleted. It is not a serialisation
 * rule, it is a rule about how a *reference* is written, and every gesture that
 * writes one needs it.
 */
