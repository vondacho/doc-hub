 ...
 *
 * The board on screen gets its colours from Tailwind classes and the custom
 ...
 * ## Read, not copied
 *
 * The four kinds — eight values with their edges, and sixteen across both
 * themes — are read off `:root` at export time rather than restated here.
 * A second copy of the notation's colours is a copy that drifts, and the drift
 * would be invisible: the board would stay right and only the exported pictures
...
import { CARD_KINDS, type CardKind } from '../examplemap/model.ts';
...
/**
 * Everything a drawing of this map needs, resolved to sRGB.
 *
...
export interface Palette {
	...
	/** The page the map is drawn on. Never transparent — see above. */
