 ...
 * story map and reading an example map.
 *
...
export function tokenize(
	source: string,
	problems: Problem[],
	keywords: ReadonlySet<string> = EXAMPLEMAP_KEYWORDS,
): readonly Token[] {
	const tokens: Token[] = [];
	...
	if (text.includes('\u0000')) {
		report(problems, {
			...
			column: 1,
			length: 0,
			hint: 'An example map is plain text. Pick an `.examplemap` file exported from this board.',
		});
	...
	if (text.length > MAX_SOURCE_BYTES) {
		report(problems, {
			...
			column: 1,
			length: 0,
			hint: 'An example map is a handful of cards. A session that produced a megabyte of them was not an example mapping session.',
		});
		return [eof(text.length, 1, 1)];
	}
