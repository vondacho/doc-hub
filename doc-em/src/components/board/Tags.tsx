 ...
 *
 * A tag is a free label — `+legal`, `+risk`, `+"needs the payments team"` —
 * and every card kind takes any number of them. What one *is*, and why the
 * vocabulary is open and the chips are monochrome, is argued once in
 * src/lib/examplemap/model.ts beside `tagKey`; this file is only how it looks.
 *
...
import { tagKey } from '../../lib/examplemap/model.ts';
...
export function Tags({
	...
	return (
		<button
			...
			className="mt-[0.3em] flex flex-wrap items-center gap-[0.25em] rounded-sm text-left focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-brand"
		>
			{tags.map((tag) => (
				<span
					key={tag}
					className="rounded-full border border-ink/25 px-[0.4em] py-px text-[0.66em] font-medium whitespace-nowrap text-ink-muted dark:border-slate-500 dark:text-slate-400"
				>
					{tag}
				</span>
			))}
...
function TagLine({
	...
	return (
		<input
			...
			className="mt-[0.3em] w-full rounded-sm bg-white/70 px-[0.2em] py-[0.1em] text-[0.72em] text-ink focus-visible:outline-2 focus-visible:outline-brand dark:bg-black/30 dark:text-slate-100"
		/>
	);
}
