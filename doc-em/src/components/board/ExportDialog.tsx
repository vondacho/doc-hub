/**
 * Where this map goes — one dialog, several destinations, one press.
 *
 * The toolbar has one Export button and it now opens this rather than writing a
 * file. That is the whole point: the number of things a map can usefully
 ...
 * wants genuinely combine: the feature file goes in the test suite, the outline goes in
 * the ticket, the `.examplemap` is the copy that survives a cleared browser,
 * and somebody finishing a session wants all three. Ticking three boxes and
 ...
 *
 * ## One row warns about itself
 *
 * The feature file is the only destination that can silently produce *less*
 * than the map holds: an open question has no Gherkin, so a map with red cards
 * on it writes a `.feature` quietly missing every one of them. That count is
 * discovered by diffing two files, which is to say discovered too late — so the
 * board passes it in and the row states it before anything is written. It is
 * the same fact GherkinDialog puts under its preview, said at the other moment
 * somebody could act on it.
 *
 * A general mechanism rather than a special case in the markup: `caveats` is
 * keyed by destination, and the row renders whatever is there. A second
 * destination that needs to warn about itself adds a line to the board, not a
 * branch to this file.
 *
 * ## Two groups, because two of the rows are not about this map
 *
 * The notation and the doctrine are reference documents: constant text, the
 * same bytes from any board, meant to be handed to an agent working on a map
 ...
 * off under their own heading because a flat list of seven would invite reading
 * "Outline" and "Doctrine" as two renderings of the same map.
 *
 ...
 * purpose. "This map" needs no explanation in a dialog opened from a map.
 ...
 * document about the map they are looking at.
 */
const GROUPS: readonly { group: Group; title: string; blurb: string | null }[] = [
	{ group: 'map', title: 'This map', blurb: null },
	{
		...
		title: 'The practice',
		blurb:
			'Not about this map. Instructions to hand a coding agent working on a .examplemap file somewhere else — the same text this board’s own assistant is given.',
	},
];
...
export function ExportDialog({
	...
	dark: boolean;
	/**
	 * How many cards the tag filter is turning down, or `null` when nothing is
	 * filtered.
	 *
	...
	/**
	 * A warning to print under one row, keyed by destination.
	 *
	 * Absent for most of them. See the note at the top on why the feature file
	 * has one and why this is a map rather than a prop named after it.
	 */
	caveats?: Partial<Record<DestinationId, string>>;
	/**
	 * Destinations that cannot run right now, with the reason, keyed the same way.
	 *
	 * The row is drawn disabled and says why, rather than being hidden. A control
	 * that vanishes when the source has a typo in it is a control somebody
	 * concludes was never there; one that says "the source does not parse" points
	 * at the pane where the problem is.
	 */
	 ...
	 *
	 * Otherwise the tick stays on, invisibly, against a filter that no longer
	 ...
	 *
	 * The selection survives the dialog closing — that is deliberate — so a
	 * feature file ticked while the source parsed is still ticked after somebody
	 * breaks the source. Filtering here rather than clearing the tick means the
	 * choice comes back when the typo is fixed, which is what the person meant.
	 */
	...
	const run = async () => {
		...
		try {
			...
			// the map is on disk, and it is not.
	...
	return (
		...
		>
			...
			<div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
						...
						<ul className="flex flex-col gap-1">
							{DESTINATIONS.filter((entry) => entry.group === group).map((entry) => (
								<li key={entry.id}>
									...
									<label
										className={`flex items-start gap-3 rounded-lg border px-3 py-2.5 transition-colors motion-reduce:transition-none ${
											unavailable[entry.id] !== undefined
									...
									>
										<input
											type="checkbox"
											checked={picked.has(entry.id) && unavailable[entry.id] === undefined}
											...
											disabled={running || unavailable[entry.id] !== undefined}
										...
										<span className="min-w-0 flex-1">
											...
											{/* The reason it cannot run outranks the warning about
											    what it would lose: one is about now, the other is
											    about a file that is not going to exist. */}
											{unavailable[entry.id] !== undefined ? (
				...
				{/*
				 * The filter, as one tick under the list rather than one per row.
				 *
				...
				<div className="mt-3 border-t border-slate-200 pt-3 dark:border-slate-700">
					...
					>
						...
						<span className="min-w-0 flex-1">
							...
							<span className="mt-0.5 block text-sm text-ink-muted dark:text-slate-400">
								{filtered === null
									? 'Nothing is filtered — the tag row is showing every card.'
									: `The pictures and the outline leave out the ${filtered.hidden} of ${filtered.total} ${
											filtered.total === 1 ? 'card' : 'cards'
										} the filter is turning down. The source file always carries the whole map.`}
							</span>
						</span>
					</label>
				</div>
